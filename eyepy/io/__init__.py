...
from eyepy import EyeMeta, EyeVolume, EyeEnface, EyeLayer, EyeVolumePixelAnnotation
...
def import_heyex_xml(path):
    ...
    layers = {key: EyeLayer(val, key) for key, val in layer_height_maps.items()}
...
def import_duke_mat(path):
    ...
    layers = {
        names[i]: EyeLayer(np.flip(height_map, axis=0), name=names[i])
